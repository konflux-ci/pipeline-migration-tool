...
* Restrict which image repositories are handled using --allowlist. Each
  value is a glob pattern matched against the image repository (Renovate
  depName). Can be specified multiple times.
  If omitted, images from arbitrary image organizations are handled.

    pmt migrate -u '<upgrades>' \\
      --allowlist 'quay.io/konflux-ci/**' --allowlist 'quay.io/app-team/**'
"""
...
def register_cli(subparser) -> None:
    ...
    migrate_parser.add_argument(
        ...
        metavar="GLOB",
        action="append",
        help="A glob pattern matching allowed image repositories (Renovate depName). "
        "'*' matches within a single path segment, '**' matches any characters "
        "including '/'. Can be specified multiple times. "
        "Only quay.io images are handled regardless of this option. "
        "If omitted, all quay.io images are accepted.",
    )
...
def _action_impl(args) -> None:
    ...
    if upgrades_data:
        upgrades = clean_upgrades(upgrades_data, allowlist=args.allowlist)
        if upgrades:
            ...
            logger.warning(
                "Input upgrades does not include bundles the migration tool aims to handle."
            )
            if args.allowlist:
                logger.warning(
                    "No upgrades matched the specified allowlist patterns: %s",
                    args.allowlist,
                )
            logger.warning(
                "The upgrades should represent task bundles generated by "
                "the Renovate tekton-bundle manager."
            )
    else:
        logger.info(
