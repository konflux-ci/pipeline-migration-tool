...
class TaskBundleUpgradesManager:
    ...
    def _collect(self, upgrades: list[dict[str, Any]]) -> None:
        for upgrade in upgrades:
            ...
            try:
                parse_version(task_bundle_upgrade.current_value)
                parse_version(task_bundle_upgrade.new_value)
            except InvalidVersion as e:
                logger.warning(
                    f"Skipping bundle upgrade due to invalid version: {e}. "
                    f"From: {task_bundle_upgrade.current_bundle}, "
                    f"To: {task_bundle_upgrade.new_bundle}"
                )
                continue
