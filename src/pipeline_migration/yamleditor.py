...
from pipeline_migration.utils import load_yaml, create_yaml_obj
...
class EditYAMLEntry:
    ...
    def insert(self, path: YAMLPath, data: Any):
        ...
        assert isinstance(last_node, (dict, list))

        yaml_str = self._gen_yaml_str(
            data, last_node.lc.col,
            seq_block=True if isinstance(last_node, list) else False
        )
        ...
        insert_text_at_line(self.yaml_file_path, lineno, yaml_str, validation_callback=post_test_yaml_validity)
    ...
    def replace(self, path: YAMLPath, data: Any):
        ...
        if len(path_stack) > 1:
            ...
            assert isinstance(parent_node, (dict, list))
            col = parent_node.lc.col

        yaml_str = self._gen_yaml_str(
            data, col,
            seq_block=True if isinstance(parent_node, list) else False
        )
        ...
        remove_lines_from_file(self.yaml_file_path, lineno, remove_lines_num, validation_callback=post_test_yaml_validity)
        insert_text_at_line(self.yaml_file_path, lineno, yaml_str, validation_callback=post_test_yaml_validity)
        self.invalidate_yaml_data()
...
def remove_lines_from_file(file_path: str, start_line: int, num_lines: int, validation_callback=None) -> None:
    """
            ...
            with os.fdopen(temp_fd, 'w', encoding='utf-8') as temp_file:
                with open(file_path, 'r', encoding='utf-8') as original_file:
                    ...
                    if start_index >= current_line:
                        raise ValueError(f"start_line ({start_line}) is beyond the file length (max index: {current_line - 1})")
...
def insert_text_at_line(file_path: str, line_number: int, text_to_insert: str, validation_callback=None) -> None:
    """
    ...
    Args:
        ...
        replace (bool): If False (default), insert before the line. If True, replace
                       a number of lines equal to the number of lines in text_to_insert.
