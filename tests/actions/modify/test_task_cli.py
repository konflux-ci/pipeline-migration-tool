...
class TestModifyTaskWithGitAdd:
    """Test git-add functionality (mocked)."""

    def test_git_add_flag_add_param(self, component_pipeline_dir, monkeypatch):
        """Test that git-add flag works with add-param command."""
        git_index = []

        def mock_git_add(*args, **kwargs):
            # Mock the subprocess.run call for git add
            git_index.append(args)

        monkeypatch.setattr("subprocess.run", mock_git_add)
        ...
        # Verify parameter was added
        pipeline_file = component_pipeline_dir.tekton_dir / "pipeline.yaml"
        verify_param_added(pipeline_file, "clone", "timeout", "30m")
    ...
    def test_git_add_flag_remove_param(self, component_pipeline_dir, monkeypatch):
        """Test that git-add flag works with remove-param command."""
        git_index = []
